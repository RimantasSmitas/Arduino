...
void loop() {
  ...
  showProgramCleanUp(1500); // clean up
 
  
  showProgramShiftSinglePixel(CRGB::Maroon, 50);

  delay(250);
/*  
  
  showProgramCleanUp(2500); // clean up
  showProgramRandom(10, 500); // show "random" program
  
  showProgramCleanUp(2500); // clean up
  showProgramShiftSinglePixel(CRGB::Blue, 100); // show "shift single pixel program" with blue pixel
  
  showProgramCleanUp(2500); // clean up // show "shift single pixel program" with maroon pixel
  
  showProgramCleanUp(2500); // clean up
  showProgramShiftMultiPixel(250); // show "shift multi pixel" program
